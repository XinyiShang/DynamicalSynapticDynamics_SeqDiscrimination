function [r,s] = run_network_noDepression(params,Iapp,init_type,memoptimize)
% network without depression and facilitation
...
switch memoptimize
    case 'yes' % D and s are only vectors not matricies
        ...
        for t=1:nTimesteps
            ...
            ds(erange) = (params.dt/params.tau_s_e)*(-s(erange) + params.alpha*params.p0_e*r(erange,t)*params.tau_s_e.*(1-s(erange)));
            dr(irange) = (params.dt/params.tau_r)*(-r(irange,t) + (params.r_i_max./(1+exp((params.theta_i - I(irange))/params.Delta_i))));
            ds(irange) = (params.dt/params.tau_s_i)*(-s(irange) + params.alpha*params.p0_i*r(irange,t)*params.tau_s_i.*(1-s(irange)));
