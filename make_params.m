...
p.tau_D = .3;  % (500ms) depression time constant
p.tau_F = .3;  % (300ms) facilitation time constant
...
p.r_i_max = 75; % (200Hz) maximum firing rate for inhibitory neurons
...
%switch p0_e_type
    %case 1 
        p.p0_e = .5;  % fraction of docked vesicles released per spike for EE/EI connections
    %cas5e 2
        %p.p0_e = .25;  % fraction of docked vesicles released per spike for EE/EI connections
...
p.stim_int = round((max_int - min_int)*rand(1,p.sequence_length+1) + min_int); %normally 1.5
