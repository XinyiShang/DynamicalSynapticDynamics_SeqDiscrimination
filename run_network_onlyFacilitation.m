function [r,s,F] = run_network_onlyFacilitation(params,Iapp,init_type,memoptimize)
% network only with synaptic facilitation
...
switch memoptimize
    case 'yes' % D and s are only vectors not matricies
        ...
        for t=1:nTimesteps
            ...
            ds(erange) = (params.dt/params.tau_s_e)*(-s(erange) + pr_e*params.alpha*r(erange,t)*params.tau_s_e.*(1-s(erange)).* F(erange));
            ...
            pr_i = params.p0_i; 
            ds(irange) = (params.dt/params.tau_s_i)*(-s(irange) + pr_i*params.alpha*r(irange,t)*params.tau_s_i.*(1-s(irange)).*F(irange));
    ...
    case 'no' % D and s are matrices
        ...
        for t=1:nTimesteps
            ...
            ds(erange) = (params.dt/params.tau_s_e)*(-s(erange,t) + pr_e*params.alpha*r(erange,t)*params.tau_s_e.*(1-s(erange,t)).*F(erange,t));
            ...
            pr_i = params.p0_i; 
            ds(irange) = (params.dt/params.tau_s_i)*(-s(irange,t) + params.alpha*params.p0_i*r(irange,t)*params.tau_s_i.*(1-s(irange,t)).*F(irange,t));
